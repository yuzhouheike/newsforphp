...
use phpQuery;
use function pq;
use function Symfony\Component\HttpFoundation\time;
use function view;
...
class ApiController extends LewaController {
    ...
    public function userRegister() {
        ...
        /* if (empty($postJson->source_id)) {
          ...
          if (is_null($token_cache)|| empty($token_cache)) {
          $post = json_encode($postData);
          $header = array('Content-Type: application/json', 'Content-Length: ' . strlen($post));
          $result = parent::curl_get($ApiUrl, $header, "POST", $post);
          parent::set_err_return($result);
    ...
    public function sendnews() {
        ...
        if ($categories == "subscribed") {
            ...
            for ($i = 0; $i < count($result["articles"]); $i++) {
                if (!empty($result["articles"][$i])) {
                    if (array_key_exists("type", $result["articles"][$i])) {
...
//删除媒体
    ...
    public function likeNews() {
        ...
        $token = $postData->token;
        $news_id = $postData->id;
        $likeResult = $this->favourModel->getUserLikeForId($news_id, $token);
        if (!is_null($likeResult)) {
            if ($likeResult->like == "true") {
                $this->favourModel->recordUserLike($news_id, $token, "false", "false");
                $this->favourModel->minusLike($news_id);
                Api::set_message("OK");
                Api::set_key("result_object");
                Api::response(array("status" => "OK"));
            }
        }
    ...
    public function unlikeNews() {
        ...
        $likeResult = $this->favourModel->getUserLikeForId($news_id, $token);
        if (!is_null($likeResult)) {
            if ($likeResult->unlike == "true") {
                $this->favourModel->recordUserLike($news_id, $token, "false", "false");
                $this->favourModel->minusUnLike($news_id);
                Api::set_message("OK");
                Api::set_key("result_object");
                Api::response(array("status" => "OK"));
            }
